...
class Tree extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const dragOverNodeKey = nextProps.dragOverNodeKey
    if(dragOverNodeKey) {
      st.dragOverNodeKey = dragOverNodeKey;
    }
    this.setState(st);
  }
  ...
  onDragEnter(e, treeNode) {
    ...
    if (this.dragNode && this.dragNode.props.eventKey === treeNode.props.eventKey && enterGap === 0 ) {
      this.setState({
        dragOverNodeKey: '',
      });
      return;
    ...
    const st = {}
    this.props.dragOverNodeKey ? null : st.dragOverNodeKey=treeNode.props.eventKey
  ...
  onDrop(e, treeNode) {
    ...
    if (this.dragNodesKeys && this.dragNodesKeys.indexOf(key) > -1) {
      if (console.warn) {
  ...
  onSelect(treeNode) {
    ...
    let selected;
    // 单选且有选中项时 不执行onSelect回调
    // 16/12/11补充：注释下面两行后可重复执行回调
  ...
  onTreeNodeEdit(e, treeNode) {
    this.props.onTreeNodeEdit({ value: e.target.value, node: treeNode });
  }
